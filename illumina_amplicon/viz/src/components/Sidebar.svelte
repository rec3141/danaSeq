...
<aside class="fixed inset-x-0 bottom-0 z-40 flex max-h-[55vh] transform flex-col
              overflow-y-auto overscroll-contain rounded-t-xl border-t border-line
              bg-surface shadow-2xl transition-transform duration-200
              ...
              md:border-line md:bg-surface/60 md:shadow-none
              {open ? 'translate-y-0' : 'translate-y-full'}">
  {#if onClose}
    <!-- Sticky so the handle stays reachable however far the filters scroll. -->
    <div class="sticky top-0 z-10 flex items-center justify-between border-b border-line
                bg-surface px-3 py-2 md:hidden">
      <span class="text-xs font-semibold uppercase tracking-wider text-muted">Filters</span>
      <button type="button" onclick={onClose} aria-label="Close filters"
              class="rounded px-3 py-1 text-lg leading-none text-muted hover:text-strong">&times;</button>
    </div>
  {/if}
      ...
      <p class="text-[11px] text-muted">
        ...
        {#if store.runInfo.portal_url}
          ...
          <a href={store.runInfo.portal_url} target="_blank" rel="noopener"
             class="hover:text-link">run {store.runInfo.slug}</a>
        {/if}
      </p>
    {/if}
    <p class="text-xs text-faint">{store.samples.length} samples | {store.asvs.length} ASVs</p>
  </div>

  <!-- ══ Taxonomy Filters (shared) ══ -->
  <div class="border-b border-line">
    <button class="flex w-full items-center justify-between px-3 py-2 text-xs font-semibold uppercase tracking-wider text-muted hover:text-fg" onclick={() => toggle('taxonomy')}>
      Taxonomy
    ...
    {#if sections.taxonomy}
      <div class="space-y-3 px-3 pb-3">
        ...
        <label class="block">
          ...
          <select bind:value={filters.colorMode} class="mt-1 w-full rounded border border-line bg-raised px-2 py-1 text-sm text-fg"
            onchange={(e) => {
        ...
        {#if filters.colorMode === 'cluster'}
          {#if store.heatmap?.sampleClusters && (activeTab === 'samples' || activeTab === 'heatmap')}
            <label class="block">
              <span class="text-xs text-muted">Sample clusters (k): {filters.sampleClusterK}</span>
          ...
          {#if store.heatmap?.asvClusters && (activeTab === 'network' || activeTab === 'phylogeny' || activeTab === 'heatmap')}
            <label class="block">
              <span class="text-xs text-muted">ASV clusters (k): {filters.asvClusterK}</span>
        ...
        {:else if filters.colorMode === 'group'}
          <fieldset class="space-y-1">
            <legend class="text-xs text-muted">Groups</legend>
        ...
        {:else}
          ...
          <div class="space-y-0.5 max-h-48 overflow-y-auto">
            ...
            <div class="mb-1 flex items-baseline gap-2 text-[10px] text-faint">
  ...
  {#if assays.length === 1}
    <div class="border-b border-line px-3 py-2">
      ...
      {#if assays[0].fwd || assays[0].rev}
        <p class="break-all text-[10px] text-faint">{[assays[0].fwd, assays[0].rev].filter(Boolean).join(' / ')}</p>
  ...
  {:else if assays.length > 1}
    <div class="border-b border-line">
      <button class="flex w-full items-center justify-between px-3 py-2 text-xs font-semibold uppercase tracking-wider text-muted hover:text-fg" onclick={() => toggle('assay')}>
        Assay
      ...
      {#if sections.assay}
        <div class="space-y-2 px-3 pb-3">
          ...
          {#if filters.assays?.size}
            <button class="text-[11px] text-link hover:text-link2"
  ...
  {#if activeTab === 'samples' || activeTab === 'network'}
    <div class="border-b border-line">
      <button class="flex w-full items-center justify-between px-3 py-2 text-xs font-semibold uppercase tracking-wider text-muted hover:text-fg" onclick={() => toggle('samples')}>
        Sample Filters
      ...
      {#if sections.samples}
        <div class="space-y-3 px-3 pb-3">
          <label class="block">
            <span class="text-xs text-muted">Min reads: {(filters.minReads || 0).toLocaleString()}</span>
          ...
          {#if activeTab === 'samples'}
            <label class="block">
              <span class="text-xs text-muted">Point scale: {fmtScale(effScale(filters.pointScale, store.maxPointScale))}x</span>
              <input type="range" min="0" max="100" step="1"
  ...
  {#if activeTab === 'network' || activeTab === 'samples'}
    <div class="border-b border-line">
      <button class="flex w-full items-center justify-between px-3 py-2 text-xs font-semibold uppercase tracking-wider text-muted hover:text-fg" onclick={() => toggle('network')}>
        Network Filters
      ...
      {#if sections.network}
        <div class="space-y-3 px-3 pb-3">
          <label class="block">
            <span class="text-xs text-muted">Min prevalence: {filters.minPrevalence || 0}</span>
            <input type="range" min="0" max="100" step="1"
          ...
          {#if store.network?.edges?.length > 0}
            <label class="block">
              <span class="text-xs text-muted">Correlation threshold: {(filters.corrThreshold || 0.3).toFixed(2)}</span>
          ...
          {#if activeTab === 'network'}
            <label class="block">
              <span class="text-xs text-muted">ASV point scale: {fmtScale(effScale(filters.networkPointScale, store.maxNetworkPointScale))}x</span>
              <input type="range" min="0" max="100" step="1"
  ...
  {#if activeTab === 'phylogeny'}
    <div class="border-b border-line">
      <button class="flex w-full items-center justify-between px-3 py-2 text-xs font-semibold uppercase tracking-wider text-muted hover:text-fg" onclick={() => toggle('phylogeny')}>
        Phylogeny
      ...
      {#if sections.phylogeny}
        <div class="space-y-3 px-3 pb-3">
          <label class="block">
            ...
            <select bind:value={filters.treeLayout} class="mt-1 w-full rounded border border-line bg-raised px-2 py-1 text-sm text-fg">
          ...
          <fieldset class="space-y-1">
            <legend class="text-xs text-muted">Tip labels</legend>
          ...
          <label class="block">
            <span class="text-xs text-muted">Min bootstrap: {filters.treeMinBootstrap || 0}%</span>
  ...
  {#if activeTab === 'heatmap'}
    <div class="border-b border-line">
      <button class="flex w-full items-center justify-between px-3 py-2 text-xs font-semibold uppercase tracking-wider text-muted hover:text-fg" onclick={() => toggle('heatmap')}>
        Heatmap
      ...
      {#if sections.heatmap}
        <div class="space-y-3 px-3 pb-3">
          <label class="block">
            <span class="text-xs text-muted">Min max(RA): {(filters.heatmapMinMaxRA ?? 1).toFixed(1)}%</span>
            <input type="range" min="0" max="100" step="1"
          ...
          <label class="block">
            <span class="text-xs text-muted">Cell size: {filters.heatmapCellSize}px</span>
