<script>
  ...
  // Per-species SAR payload, loaded lazily on mount. Shape:
  //   { zebra_mussel: { counts: {...}, stats: {...}, identity_bins: {...} } }
  // File-name convention: ./data/sar_<species.id>.json[.gz].
  let sarPayloads = $state({});
  let sarLoaded = $state(new Set());

  async function loadSarCounts(speciesId) {
    if (sarLoaded.has(speciesId)) return;
    sarLoaded.add(speciesId);
    const base = (import.meta.env.BASE_URL || '/').replace(/\/$/, '');
    const url = `${base}/data/sar_${speciesId}.json`;
    try {
      ...
      sarPayloads = { ...sarPayloads, [speciesId]: payload };
    } catch (e) {
      console.warn(`SAR counts for ${speciesId} not available:`, e.message);
      sarPayloads = { ...sarPayloads, [speciesId]: { counts: {}, stats: {} } };
    }
  }

  let activePayload = $derived(sarPayloads[species.id] || { counts: {}, stats: {} });
  ...
  $effect(() => { loadSarCounts(species.id); });
  ...
  const metricGroup = $derived({ values: ['sar_hits', 'sar_hq_hits', 'sar_fraction', 'sar_hq_fraction', 'read_count', 'diversity', 'cluster'], labels: ['SAR hits', `HQ hits (>${hqCutoff}%)`, 'SAR / read', 'HQ / read', 'Reads', 'Diversity', 'Heatmap Cluster'] });
  // Default sizing for /sar is the SAR-hit count — that's the whole point of
  // the page. `sar_fraction` is hits / sample_read_count (relative abundance).
  const sizeGroup =   $derived({ values: ['sar_hits', 'sar_hq_hits', 'sar_fraction', 'sar_hq_fraction', 'fixed', 'read_count', 'total_bases', 'diversity'], labels: ['SAR hits', `HQ hits (>${hqCutoff}%)`, 'SAR / read', 'HQ / read', 'Fixed', 'Reads', 'Bases', 'Diversity'] });
  ...
  function cycleSpecies() { speciesIdx = (speciesIdx + 1) % SAR_SPECIES.length; }

  // SARView defaults: color by SAR abundance, size by SAR hits.
  let colorMode = $state('metric'); // 'info' | 'taxonomy' | 'metric' | 'metadata'
  let infoField = $state('flowcell');
  let metricField = $state('sar_hits');
  let metaField = $state('');

  let sizeBy = $state('sar_hits');
  ...
  let colorIsContinuous = $derived.by(() => {
    ...
    return ['read_count', 'total_bases', 'diversity', 'sar_hits', 'sar_hq_hits', 'sar_fraction', 'sar_hq_fraction'].includes(colorBy);
  });
  ...
  let allMarkers = $derived.by(() => {
    ...
    const markers = $samples.map(s => {
      ...
      const marker = {
        ...
        sar_hq_hits,
        sar_hq_fraction,
        sar_mean_identity: stat?.mean_identity ?? null,
        sar_mean_mapq: stat?.mean_mapq ?? null,
        sar_identity_hist: stat?.identity_hist ?? null,
        ...m,
      };
      if (taxSubTaxa) {
  ...
  let selectedSar = $derived(activeStats[$selectedSample ?? ''] || null);
  ...
  let tableColumns = $derived([
    ...
    { key: 'sar_hits', label: 'SAR hits', render: v => v ? v.toLocaleString() : '-' },
    { key: 'sar_hq_hits', label: `HQ hits (>${hqCutoff}%)`, render: v => v ? v.toLocaleString() : '-' },
    { key: 'sar_fraction', label: 'SAR / read', render: fmtFraction },
  ...
  {/if}
    ...
    <div class="flex items-center gap-2 flex-wrap text-xs">
      ...
      <button
        ...
        title={`SAR species — click to cycle (${SAR_SPECIES.length} available)`}
      >
    ...
    <div class="flex gap-6">
      ...
      <div class="flex-1 h-[500px] rounded-lg overflow-hidden border border-slate-700">
        <LeafletMap
          ...
          {dimIds}
          exportName={`danaseq_sar_${species.id}_color-${colorBy}`}
        />
      </div>
      ...
      <div class="w-72 shrink-0 bg-slate-800 rounded-lg border border-slate-700 p-4 space-y-3 h-fit max-h-[500px] overflow-y-auto">
        {#if selectedDetail}
          ...
          <!-- SAR hit-quality summary + identity histogram. Bars span the
          ...
          <div class="border border-emerald-400/30 bg-emerald-400/5 rounded-md p-2 space-y-1">
            <div class="flex items-baseline justify-between text-xs">
              ...
              <span class="text-slate-400">
                {selectedSar ? selectedSar.n.toLocaleString() : 0} hits
                {#if selectedSar?.hq_hits}<span class="text-rose-300 ml-1">({selectedSar.hq_hits.toLocaleString()} HQ)</span>{/if}
              </span>
            </div>
            {#if selectedSar && selectedSar.n > 0}
              <div class="text-[10px] text-slate-400 flex justify-between">
                <span>mean id <span class="text-slate-200 font-mono">{selectedSar.mean_identity.toFixed(1)}%</span></span>
                <span>mean mapq <span class="text-slate-200 font-mono">{selectedSar.mean_mapq.toFixed(1)}</span></span>
              </div>
              {@const hist = selectedSar.identity_hist}
              ...
              {#if selectedSar.pos_hist}
                {@const ph = selectedSar.pos_hist}
    ...
    <div class="flex flex-wrap items-baseline gap-x-6 gap-y-1 text-xs px-2">
      ...
      <span class="text-emerald-300">SAR / read <span class="text-slate-100 font-mono">{fmtFraction(tableTotals.fraction)}</span></span>
    ...
    <DataTable
      ...
      idKey="id"
      maxHeight="300px"
      exportFilename={`sar_${species.id}_samples`}
