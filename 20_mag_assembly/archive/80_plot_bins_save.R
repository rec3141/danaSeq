...
prjdir = "/data/project_CMO2025/map_and_bin_20251121-150940/"
setwd(file.path(prjdir))

metadata_file <- "CMO\ 2025\ MICB.xlsx"
...
eval.in = read_tsv(file.path(prjdir,"dastool_DASTool_bins/dastool_DASTool_summary.tsv"),col_names=T)
eval.in$bin = paste0("B_",eval.in$bin)
eval.in$taxstring = unname(z[eval.in$bin])
...
h1=heatmap.2((depth.bins.prop)^.25,trace="none", col=viridis,margins=c(20,100),
          hclustfun=function(x) hclust(x, method="ward.D"),
          distfun=function(x) vegdist(x, method="bray"),
          labRow = collab, cexRow = 2,
          labCol = rowlab, cexCol = 2,
          RowSideColors = rsc,
...
for(samp in depth.sample.hclust$labels[depth.sample.hclust$order]) {
  ...
  plot(tnfs.tsne$Y[,1:2],pch='.',cex=0.1,col='gray15', 
       ...
       bty  = "n",                  # no box
       main=fcbc[samp,"sample"], col.main="white"
  )
...
for(bin in 1:ncol(depth.bins.prop)) {
  ...
  plot(tnfs.tsne$Y[,1:2],pch='.')
  points(tnfs.tsne$Y[pick.contigs,1:2],pch=21,col="blue")
