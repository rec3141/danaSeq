...
setwd("/data/project_CMO2025/map_and_bin_20251121-150940")
mat.in=read.delim("bamdir/depths_jgi.txt",row.names=1)
mat = mat.in[,seq(3,ncol(mat.in),2)]

tsne = Rtsne(mat, check_duplicates=F)

plot(tsne$Y[,1:2],pch='.',bg=NULL,cex=2,col=apply(mat,1,which.max))


###

#system("head -n4 Tetra_all.lrn | tail -n1 | cut -f2- -d' ' > tnfs.txt")
tnfs = as.character(read.table("tetra/tnfs.txt",stringsAsFactors=F))
tnfs = tnfs[2:length(tnfs)]

lrn = read_table("tetra/Tetra_all.lrn",comment="%",col_names=F)
lrn$X1 = NULL
