...
declare -A FLOWCELL_TO_RUNID
declare -A FLOWCELL_TO_DESTDIR

# Function to build the mapping of flowcell IDs and run IDs
build_mapping() {
    echo "Scanning destination directories to build flowcell/run ID mapping..."
            ...
            for subdir in "$run_dir"/*; do
                if [ -d "$subdir" ]; then
                    ...
                    if [[ "$subdir_name" =~ ([^_]+)_([^_]+)_([^_]+)_([^_]+)_([^_]+)$ ]]; then
                        ...
                        FLOWCELL_TO_RUNID["$flowcell_id"]="$short_run_id"
                        FLOWCELL_TO_DESTDIR["$flowcell_id"]="$subdir/fastq_pass"
                        
                        echo "  Found: $flowcell_id -> $short_run_id (in $run_name)"
                    fi
                fi
            done
        fi
    ...
    if [ ${#FLOWCELL_TO_RUNID[@]} -eq 0 ]; then
        echo "Error: No valid destination directories found!"
        exit 1
    fi
    
    echo "Built mapping for ${#FLOWCELL_TO_RUNID[@]} flowcell/run ID pairs."
    echo ""
}
...
# Function to find matching flowcell for a run ID
find_flowcell_for_run() {
    local run_id="$1"
    for flowcell in "${!FLOWCELL_TO_RUNID[@]}"; do
        if [ "${FLOWCELL_TO_RUNID[$flowcell]}" = "$run_id" ]; then
            echo "$flowcell"
            return
        fi
    done
...
process_barcode() {
    ...
    for file in "$source_dir"/fastq_runid_*_*_0.fastq.gz; do
        ...
        local flowcell_id=$(find_flowcell_for_run "$run_id")
        ...
        local dest_base_dir="${FLOWCELL_TO_DESTDIR[$flowcell_id]}"
...
main() {
    ...
    if [ ! -d "$DEST_BASE" ]; then
        ...
        ls -1 /data/minknow/ 2>/dev/null | grep -E '^[A-Z0-9]+[0-9]{4}'
    
    # Build the flowcell/run ID mapping
    build_mapping
    
    # Show the mapping
    echo "Flowcell -> Run ID mapping:"
    for flowcell in "${!FLOWCELL_TO_RUNID[@]}"; do
        echo "  $flowcell -> ${FLOWCELL_TO_RUNID[$flowcell]}"
    done
    echo ""
