...
export DANADIR="/data/danaSeq/nanopore_live"
...
# Add to PATH
export PATH="${DANASEQ_ROOT}/nanopore_live:${PATH}"
export PATH="${DANASEQ_ROOT}/nanopore_mag:${PATH}"
```
...
grep -A5 "mkdir.*lockfile" nanopore_live/24_process_reads_optimized.sh
# Should show atomic lock acquisition

# Check for bash -c (not eval)
grep "eval" nanopore_live/24_process_reads_optimized.sh
# Should ONLY appear in comments, not in code

# Check for signal handlers
grep "trap.*EXIT" nanopore_live/24_process_reads_optimized.sh
# Should show cleanup handlers

# Check for error message standardization
grep '\[ERR\]' nanopore_live/24_process_reads_optimized.sh
...
- `nanopore_live/README.md` - Real-time pipeline details
- `nanopore_mag/README.md` - MAG assembly details
...
if grep -q "mkdir.*lockfile" nanopore_live/24_process_reads_optimized.sh; then
...
if ! grep -q '^[^#]*eval ' nanopore_live/24_process_reads_optimized.sh; then
...
if grep -q "trap.*cleanup_on_exit.*EXIT" nanopore_live/24_process_reads_optimized.sh; then
...
if grep -q "validate_output_dir" nanopore_mag/61_map_and_bin_optimized.sh; then
...
if ! grep -q '\[ERR\]' nanopore_live/24_process_reads_optimized.sh; then
